...
const UPI_ID=process.env.UPI_ID||'9534250702@okbizaxis';
const PAYEE_NAME=process.env.PAYEE_NAME||'Kushwaha Internet Cafe';
...
app.get('/api/config',(req,res)=>res.json({whatsapp:WHATSAPP,upi:UPI_ID,payee:PAYEE_NAME}));
...
async function sendPdfEmail(o,p){if(!RESEND_API_KEY||!EMAIL_FROM||!o.customer?.email||!p?.filePath||!fs.existsSync(p.filePath))return {sent:false,reason:'Email service not configured or PDF/email missing'};const content=fs.readFileSync(p.filePath).toString('base64');const r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{'Authorization':'Bearer '+RESEND_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({from:EMAIL_FROM,to:[o.customer.email],subject:'Your IGNOU PDF - '+p.title,html:'<p>Hello '+String(o.customer.name).replace(/[<>]/g,'')+',</p><p>Your payment has been verified. Your PDF is attached.</p><p>Order ID: '+o.id+'</p>',attachments:[{filename:p.title+'.pdf',content:content,content_type:'application/pdf'}]})});const data=await r.json().catch(()=>({}));if(!r.ok)throw Error(data.message||'Email sending failed');return {sent:true,id:data.id||null};}
app.post('/api/admin/orders/:id/status',admin,async(req,res)=>{const {status}=req.body||{};if(!['approved','rejected','pending'].includes(status))return res.status(400).json({error:'Invalid status'});const d=read();const o=d.orders.find(x=>x.id===req.params.id);if(!o)return res.status(404).json({error:'Order not found'});o.status=status;let email={sent:false};if(status==='approved'&&!o.emailSentAt){const p=d.products.find(x=>x.id===o.productId);try{email=await sendPdfEmail(o,p);if(email.sent)o.emailSentAt=new Date().toISOString();}catch(e){email={sent:false,error:e.message};}}write(d);res.json({ok:true,email});});
