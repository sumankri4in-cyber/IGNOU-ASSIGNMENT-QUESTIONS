...
app.post('/api/orders',auth,(req,res)=>{const {productId,txnRef}=req.body||{};const d=read();const p=d.products.find(x=>x.id===productId);if(!p)return res.status(404).json({error:'Product not found'});const o={id:crypto.randomUUID(),userId:req.user.id,productId,txnRef:String(txnRef||'').trim(),status:'pending',createdAt:new Date().toISOString()};d.orders.push(o);write(d);res.json({ok:true,order:o});});
